...
BLACK= (0,0,0)
# define font
font = pygame.font.SysFont('Fututra',30)
...
class Soldier(pygame.sprite.Sprite):
    ...
    def ai(self):
        if self.alive and player.alive:
            ...
            # check if the ai is near rectangle
    ...
    def move(self,moving_left,moving_right):
        ...
        self.rect.x +=dx
        self.rect.y +=dy
        # update scroll based on soldier position
    ...
    def update_animation(self):
        ...
        if pygame.time.get_ticks()-self.update_time > ANIMATION_COOLDOWN:
            self.update_time = pygame.time.get_ticks()
            self.frame_index+=1
        # if the animation has finished, reset back to the start
...
class World():
    ...
    def process_data(self,data):
        # iterate through each level in data file
            ...
            for x, tile in enumerate(row):
                if tile>=0:
                    ...
                    if tile>=0 and tile<=8:
                        ...
                        player = Soldier('player',x*TILE_SIZE,y*TILE_SIZE,1.7,6.5,20,5)
...
class Grenade(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        dy = self.vel_y
        # check for the collision with the levels
        for tile in world.obstacle_list:
            if tile[1].colliderect(self.rect.x+dx,self.rect.y,self.width,self.height):
                self.direction *= -1
                dx = self.direction*self.speed
        ...
        self.rect.y +=dy
        # check if grenade collision with walls
...
enemy = Soldier('enemy',600,300,1.7,2,20,0)
enemy2 = Soldier('enemy',500,290,1.7,2,20,0)
...
while(run):
    ...
    # show grenade
    draw_text(f'GRENADE:{player.grenades}',font,WHITE,10,62)
    player.update()
    player.draw()
